\chapter{Framework Valutati}
	...
	\label{sec:frameworkhybrid}
		\subsection{Phonegap}
			...
			usare (differentemente a seconda della piattaforma) 
	...
	\end{lstlisting}
			...
			Questo viene fatto con l'uso del metodo
	\begin{lstlisting}[language=Java]
	loadUrl(String javascriptCode)
	\end{lstlisting}			
			dove la stringa javascriptCode deve essere del tipo 
			``javascript: codice javascript da eseguire''.
						
			PhoneGap sfrutta questa funzionalità per creare le API JavaScript 
			che sono in grado di accedere alle funzionalità native del dispositivo. 
			
			%integrarci il discorso dei plugin
			...
			per l'accesso al dispositivo, e al suo interno ci racchiude 
			l'applicazione web. Il pacchetto 
		...
		\subsection{Sencha Touch}
			...
			una volta installato con il semplice comando \ref{cod:appsencha}
			si crea una nuova applicazione di nome ``MyApp'' nella cartella ``./MyAppDir''.
			\begin{lstlisting}[
				label={cod:appsencha},
				caption={Comando Bash per creare lo scheletro di un'applicazione Sencha Touch per Android.},
				language=MyBash
			]
	sencha generate app MyApp ./MyApp
			...
			le proprio preferenze è sufficiente dare il comando \ref{cod:buildsencha}
	...
	sencha app package build packager.json
			...
			Dalla struttura delle cartelle del progetto si evince che \senchat{} segue
			il modello MVC estendendolo con altre componenti traendone tutti i
