\chapter{Framework Valutati}
	...
	\label{sec:frameworkwebapp}
		...
		\subsection{Kendo UI Mobile}
			...
			Il punto di forza di Kendo UI Mobile è la 
			...
			Un'applicazione manipola certe entità, ad esempio un'app che gestisce 
			le domande di laurea dovrà trattare studenti, dove uno studente avrà 
			particolari caratteristiche come nome, cognome e numero di matricola.
			Questo avrà due rappresentazioni, una mediante strutture dati opportune 
			che modellano questa entità in memoria, e un'altra grafica dove i dati
			dello studente, saranno mostrati all'utente attraverso elementi grafici.
			L'utente può inoltre compiere determinate azioni su queste entità sempre 
			attraverso elementi grafici (es. sottoporre la domanda di laurea attraverso
			il tocco di un apposito tasto). Il paradigma Mo\-del-\-View-\-View\-Mo\-del 
			formalizza le due rappresentazioni rispettivamente come Model e come 
			View, inoltre introduce il ViewModel, un terzo elemento che le mette 
			in relazione, consentendo che le azioni e le modifiche fatte da parte dell'utente 
			sulla View si ripercuotano sul relativo Model, e viceversa.
			Kendo UI fornisce il framework MVVM proprio con lo scopo di separare 
			i dati dall'interfaccia grafica, ottenendo 
			così un'applicazione più strutturata che si traduce in codice 
			più manutenibile e più leggibile.
			...
			attributi hanno la capacità di forzare il browser ad avvisare l'utente che un certo elemento
