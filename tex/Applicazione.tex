\chapter{L'Applicazione}
	...
	function mapCompleted(evt){ ... }
		...
		\subsection{Lista proprie segnalazioni}
			...
			ovviamente dato che non ci interessa mostrare l'immagine della segnalazione
			nella lista questa non viene scaricata velocizzando così il 
			...
			campi mostrare degli oggetti inseriti nella lista. Il suddetto vettore 
			...
			Per \kendomob{} abbiamo creato una \texttt{view} con al suo interno 
			una lista \texttt{<ul>} decorata con l'attributo \texttt{data-role=listView}, 
			in \js{} abbiamo poi usato la funzione \texttt{kendoMobileListView}, 
			fornita dal framework stesso, per definire le operazioni da eseguire 
			quando viene premuto un elemento della lista e per 
	...
	httpClt.send();
        ...
        \subsection{Lista delle ultime segnalazione}
